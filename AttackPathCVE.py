...
def login(url, api_key, api_secret):
    ...
    response = requests.request("POST", api_endpoint, headers=headers, json=payload)
    return response.text

def get_alerts(params, headers):
    conn = http.client.HTTPSConnection("api2.prismacloud.io")
